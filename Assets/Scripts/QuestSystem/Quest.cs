...
public class Quest
{
	...
	private bool CheckIfQuestCompleted()
	{
		...
		{
			List<InventoryItem> missingItems = new();
	...
	public void MissingQuestItems(List<InventoryItem> missingItems)
	{
		...
		for (int i = 0; i < missingItems.Count; i++)
		{
			if (i <= missingItems.Count - 1)
				missingItemsInfo += missingItems[i].ItemDefinition.ItemName + ", ";
			else
				missingItemsInfo += missingItems[i].ItemDefinition.ItemName;
		}
